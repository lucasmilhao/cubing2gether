    ...
    return (
        <div className="amigo-container">
            {usuarios && usuarios.length > 0 ? usuarios.map((e) => {
                const seguidor = e.seguidor;
                return (
                    <UsuarioCard
                        key={seguidor.id}
                        usuario={seguidor}
                        actions={seguidor.id !== usuarioLogado?.id ? (
                            <>
                                <button className="chat-button" onClick={() => {
                                    idsUsuarios.push(seguidor.id);
                                    if (usuarioLogado?.id) {
                                        idsUsuarios.push(usuarioLogado.id);
                                    }
                                    submitPartida(`${seguidor.nome} e ${usuarioLogado?.nome}`, idsUsuarios);
                                ...
                                <button className="chat-button" onClick={() => {
                                    ...
                                    submitConversa(`${seguidor.nome} e ${usuarioLogado?.nome}`, idsUsuarios)
